...
import {
  BarChart2, TrendingUp, FileDown, Users,
  MapPin, Building2, CheckCircle, Hammer, type LucideProps,
} from 'lucide-react';
import {
...
const sl = (key: string): string => STAGE_LABELS[key] || key.replace(/_/g, ' ');
...
const exportCSV = (stats: any) => {
  ...
  const rows: any[][] = [
    ...
    // Regional
    ...(am.region_data ?? []).map((r: any) => ['Region', r.name, r.count]),
  ];
...
const SectionHeader = ({ icon: Icon, title, sub }: { icon: ComponentType<LucideProps>; title: string; sub?: string }) => (
...
const Reports = () => {
  ...
  // role/id come from the JWT in the request header — no args needed
  const { data: stats = null, isLoading: loading, error: queryError } = useProjectStats();
  const error = queryError ? 'Could not load report data. Check your connection.' : null;

  if (!user) return null;

  // ── Derived values ──────────────────────────────────────────────────────────
  const sb: Record<string, number>   = stats?.stage_breakdown ?? {};
  const rc: Record<string, number>   = stats?.role_counts     ?? {};
  const am: Record<string, any>      = stats?.admin_metrics   ?? {};
  const kpis: Record<string, any>    = am.kpis                ?? {};

  const totalStages = Object.values(sb).reduce((a: number, b: number) => a + b, 0);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="kpi-strip animate-delay-1">
            {summaryKPIs.map(({ icon: Icon, label, value, color }: any) => (
          ...
          <div className="glass-card-dark report-section animate-delay-2" style={{ marginBottom: '1.5rem' }}>
            ...
            <div className="report-financials-grid">
              ...
              ].map(({ label, value, color, bold }: any) => (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', marginTop: '0.5rem' }}>
                  {STAGE_GROUPS.map(group => {
                    const items = group.keys
                      .map((k: string) => ({ key: k, count: sb[k] || 0 }))
                      .filter((x: any) => x.count > 0);
                    if (items.length === 0) return null;
                    const maxG = Math.max(...items.map((x: any) => x.count), 1);
                    return (
                      <div key={group.label}>
