...
import {
  Building2, Leaf, Cpu, AlertTriangle, Plus, Trash2,
...
const StructureCard = ({ structure, selected, onSelect }) => (
  <button
    type="button"
...
const BreakdownRow = ({ num, type, name, area, rate, eligible, subsidy, unit = 'SQM', qty }) => (
  <tr>
...
const SummaryChip = ({ label, value, color, sub }) => (
...
const SubsidyCalculator = () => {
  ...
  // ── Form state ──
  const [tab,          setTab]        = useState('structure');   // 'structure' | 'addons'
  const [structureId,  setStructureId] = useState('');
  const [areaSqm,      setAreaSqm]    = useState('4000');
  const [areaTypeId,   setAreaTypeId] = useState('');
  const [includeCrop,  setIncludeCrop] = useState(false);
  const [cropId,       setCropId]     = useState('');
  const [selComponents, setSelComponents] = useState([]);
  const [compSearch,   setCompSearch] = useState('');
  const [showCompDD,   setShowCompDD] = useState(false);
    ...
    getSubsidyRates()
      .then(data => {
        if (data?.structures?.length) {
          const mapped = data.structures.map(s => ({
            id: String(s.id),
            code: s.code || '',
            label: s.name,
        ...
        if (data?.area_types?.length) {
          const mapped = data.area_types.map(a => ({
            id: String(a.id),
            label: a.name,
  ...
  const calc = useMemo(() => {
    if (!selectedStructure?.eligible || area <= 0) {
      return { lines: [], totalEligible: 0, totalSubsidy: 0, farmerContrib: 0, inst1: 0, inst2: 0 };
    }
    ...
    const cropLine = includeCrop && selectedCrop ? (() => {
    ...
    const compLines = selComponents.map(c => {
      const elig    = c.costPerUnit * c.qty * multiplier;
      const subsidy = elig * 0.50;
      return { ...c, type: 'Component', unit: c.unit, rate: c.costPerUnit, eligible: elig, subsidy };
    });

    const lines = [structLine, cropLine, ...compLines].filter(Boolean);
  ...
  const removeComponent = (id) => setSelComponents(p => p.filter(c => c.id !== id));

  const updateQty = (id, qty) =>
    setSelComponents(p => p.map(c => c.id === id ? { ...c, qty: Math.max(1, parseInt(qty) || 1) } : c));
