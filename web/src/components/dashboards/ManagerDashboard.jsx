...
const DataRow = ({ label, value, color, to }) => (
...
const ActionAlert = ({ icon: Icon = AlertTriangle, color, bg, children }) => (
  <div style={{
...
const QueueList = ({ title, items, icon: Icon }) => (
...
const PmPanel = ({ k, stats }) => {
  if (!k) return null;
  const pmMax    = Math.max(...(k.pm_breakdown?.map(p => p.projects) || [1]), 1);
  const stageMax = Math.max(...Object.values(k.stage_breakdown || {}), 1);
        ...
        <div className="dashboard-card animate-fade-in animate-delay-3">
          ...
          <div className="card-body">
            {Object.keys(k.stage_breakdown || {}).length > 0 ? (
              <div className="bar-chart">
                {Object.entries(k.stage_breakdown)
                  .sort(([, a], [, b]) => b - a)
                  .map(([s, c]) => (
...
const AgroPanel = ({ k }) => {
  if (!k) return null;
  const agroMax = Math.max(...(k.agro_breakdown?.map(a => a.consultations) || [1]), 1);
...
const ContractorPanel = ({ stats, roleKpis, role }) => {
  ...
  const stageMax     = Math.max(...cfg.stages.map(s => breakdown[s] || 0), 1);
...
const ManagerDashboard = ({ stats, error, user }) => {
  const [roleKpis, setRoleKpis]       = useState(null);
  const [kpisLoading, setKpisLoading] = useState(false);

  const cfg        = ROLE_CONFIG[user.role] ?? { icon: '👤', label: `${user.role} Dashboard`, focus: 'Your Assignments' };
