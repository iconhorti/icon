...
export const stageLabel = (key) => STAGE_LABELS[key] || key.replace(/_/g, ' ');
...
// ─── KPI Card ─────────────────────────────────────────────────────────────────
const KpiCard = ({ icon: Icon, label, value, sub, color = 'primary', alert, onClick }) => (
  <div
...
// ─── AdminDashboard ───────────────────────────────────────────────────────────
const AdminDashboard = ({ stats, error, user }) => {
  ...
  const sumStages = (keys) => keys.reduce((s, k) => s + (stageBreakdown[k] || 0), 0);
  ...
  const openDrillDown = async (title, stageKeys) => {
    ...
    try {
      ...
      const rawData = await getProjects(params);
