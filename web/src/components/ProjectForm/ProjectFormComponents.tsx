...
interface ProjectFormComponentsProps {
  allLineItems: any[];
  getSelectedItem: (item: any) => any;
  toggleItem: (item: any) => void;
  updateQty: (tempId: any, qty: any) => void;
  totals: { eligible: number; subsidy: number };
  submitting: boolean;
  handleSubmit: () => void;
...
// Presentational rupee formatter (was previously an undefined runtime global).
const fmt = (val: any): string => `Rs.${Math.round(Number(val) || 0).toLocaleString('en-IN')}`;

export default function ProjectFormComponents({
  allLineItems, getSelectedItem, toggleItem, updateQty,
  totals, submitting, handleSubmit, setActiveTab,
  selectedItems, savedProjectId, userRole,
}: ProjectFormComponentsProps) {
  return (
