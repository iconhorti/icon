...
import type { State, District, Taluka, Village } from '../types/models';
...
interface LocationPickerProps {
  value?: number | '';
  onChange: (villageId: number | '') => void;
  required?: boolean;
  compact?: boolean;
  disabled?: boolean;
}

const LocationPicker = ({ value = '', onChange, required = false, compact = false, disabled = false }: LocationPickerProps) => {
  ...
  // ── Cascade handlers ──────────────────────────────────────────────────────────
  const handleStateChange = useCallback(async (e: ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleDistrictChange = useCallback(async (e: ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleTalukaChange = useCallback(async (e: ChangeEvent<HTMLSelectElement>) => {
  ...
  const selProps = (isDisabled: boolean) => ({
    style: { ...SEL_STYLE, opacity: isDisabled ? 0.55 : 1, cursor: isDisabled ? 'not-allowed' : 'pointer' } as CSSProperties,
