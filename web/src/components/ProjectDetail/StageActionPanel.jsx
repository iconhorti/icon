...
const StageActionPanel = ({ stageId, project, role, onSaved }) => {
  ...
  useEffect(() => {
    const PRE = {
      bank_processing:    { loan_amount: project.loan_amount || '', loan_sanction_date: project.loan_sanction_date || '', loan_account_number: project.loan_account_number || '' },
      goc_registration:   { goc_number: project.goc_number || '', goc_date: project.goc_date || '' },
      m7_plantation:      { plantation_date: project.plantation_date || '', seedlings_count: project.seedlings_count || '', agronomist_recommendations: project.agronomist_recommendations || '' },
      subsidy_claim:      { subsidy_claim_reference: project.subsidy_claim_reference || '', subsidy_claim_date: project.subsidy_claim_date || '' },
      agency_inspection:  { subsidy_inspection_date: project.subsidy_inspection_date || '', subsidy_inspector_name: project.subsidy_inspector_name || '', subsidy_inspection_remarks: project.subsidy_inspection_remarks || '', subsidy_inspection_passed: project.subsidy_inspection_passed ?? '' },
      committee_meeting:  { subsidy_meeting_date: project.subsidy_meeting_date || '', subsidy_meeting_decision: project.subsidy_meeting_decision || '', subsidy_approved_amount: project.subsidy_approved_amount || '', subsidy_meeting_remarks: project.subsidy_meeting_remarks || '' },
      // FIX: subsidy_bank_credit_date was missing — saved value was erased on re-open
      subsidy_released:   { subsidy_release_date: project.subsidy_release_date || '', subsidy_release_order_number: project.subsidy_release_order_number || '', subsidy_release_amount: project.subsidy_release_amount || '', subsidy_bank_credit_date: project.subsidy_bank_credit_date || '' },
      completed:          { completion_certificate_date: project.completion_certificate_date || '', farmer_feedback: project.farmer_feedback || '', farmer_rating: project.farmer_rating || '' },
    };
    setForm(PRE[stageId] || {});
  }, [stageId, project]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateProjectFields(project.id, form);
      setSaved(true);
  ...
  const PanelContent = panels[stageId];
...
const ConstructionPanel = ({ icon, label, extra }) => (
  <div className="stage-panel">
