...
const TeamAssignmentCard = ({ project, userRole, refresh }) => {
  ...
  useEffect(() => {
    getSkills()
      .then(data => {
        setSkills(data || []);
        if (data?.length > 0) setSelectedSkillId(String(data[0].id));
      })
      .catch(console.error)
  ...
  project.contractors?.forEach(pc => {
    ...
    <div className="glass-card detail-card animate-delay-2" style={{ marginTop: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
        <div className="input-group" style={{ flex: '1 1 180px', marginBottom: 0 }}>
          ...
          <select className="input-field" value={selectedSkillId} onChange={e => setSelectedSkillId(e.target.value)} disabled={loadingSkills}>
        ...
        <div className="input-group" style={{ flex: '2 1 250px', marginBottom: 0 }}>
          ...
          <select className="input-field" value={selectedContractorId} onChange={e => setSelectedContractorId(e.target.value)} disabled={loadingContractors}>
