...
const ColPicker = ({ columnDefs, gridRef }) => {
  const [open, setOpen]       = useState(false);
  const [hidden, setHidden]   = useState(new Set()); // set of field/colId keys that are hidden
  ...
  const colKey = (c) => c.field || c.colId || c.headerName;
...
const computeFooterRow = (rowData, columnDefs) => {
  const footer = {};
  let labelSet = false;
...
const AgTable = ({
  ...
  const selectionProp = !rowSelection
    ? undefined
  ...
  const pinnedBottom = useCallback(() => {
    ...
    <div style={{ width: '100%' }}>
      ...
      {showColPicker && !loading && (
        <div style={{
          ...
          <ColPicker columnDefs={columnDefs} gridRef={gridRef} />
        </div>
      )}
